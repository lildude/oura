...
func TestSleep(t *testing.T) {
	...
	t.Run("get sleep without specific dates", func(tc *testing.T) {
	...
	t.Run("get sleep with only start date", func(tc *testing.T) {
		client, mux, _, teardown := setup()
		defer teardown()

		mux.HandleFunc("/sleep", func(w http.ResponseWriter, r *http.Request) {
			assert.Equal(tc, http.MethodGet, r.Method)
			assert.Equal(tc, "/sleep?start=2020-01-20", r.URL.String())
			fmt.Fprint(w, mock)
		})
	...
	t.Run("get sleep with start and end dates", func(tc *testing.T) {
		client, mux, _, teardown := setup()
		defer teardown()

		mux.HandleFunc("/sleep", func(w http.ResponseWriter, r *http.Request) {
			assert.Equal(tc, http.MethodGet, r.Method)
			assert.Equal(tc, "/sleep?end=2020-01-25&start=2020-01-20", r.URL.String())
			fmt.Fprint(w, mock)
		})
